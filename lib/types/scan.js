 ...
 */
export function parseMcpToolName(publicName) {
    if (!publicName.startsWith(MCP_PREFIX))
        return undefined;
...
export function collectLoaded(schemas) {
    ...
    for (const schema of schemas) {
        ...
        if (list === undefined)
            byServer.set(parsed.serverName, [parsed.rawName]);
        else
            list.push(parsed.rawName);
    }
    return byServer;
}
 ...
 */
export function scanYamlServerRows(text, source) {
    const rows = [];
    let current;
    const flush = () => {
 ...
 */
export async function readProfileConfig(dshHome, profile) {
    const paths = [];
    ...
    for (const text of texts) {
        if (text !== undefined)
            rows.push(...scanYamlServerRows(text, 'profile'));
    }
    return rows;
}
...
export async function scan(input) {
    ...
    const [project, profileRows] = await Promise.all([
        ...
        dshHome === '' ? Promise.resolve([]) : readProfileConfig(dshHome, profile),
    ]);
    return buildReport({
        loaded,
